...
def send_message(message):
    ...
    if 'choices' in response_data:
       ...
       # Split the API response by newlines
        lines = data.split('\n')

        # Find the index of the line starting with "Day 1:"
        start_index = next((i for i, line in enumerate(lines) if line.startswith('Day 1:')), None)

        # Remove the "ChatGPT" line
        if start_index is not None:
            lines.pop(start_index + 1)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        elif " travel itenary" in query or "travel itinerary" in query:
            locations = extract_itenary(query)
            if locations is not None:
                location1, location2 = locations
                speak("How many days do you wat to spent for this trip?")
                days  = takeCommand()
                num_days = 2
                if "day" or "days" in days:
                    # Use regular expression to extract the number of days
                    if "day" in days:
                        pattern = r'(\d+) day'
                    else:
                        pattern = r'(\d+) days'
